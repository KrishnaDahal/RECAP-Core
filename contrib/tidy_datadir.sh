...
if [ -d "$1" ]; then
  ...
  echo "Removes obsolete Recap database files" >&2
  exit 1
fi
...
case $LEVEL in
  0)
    echo "Error: no Recap datadir detected."
    exit 1
    ;;
  1)
    echo "Detected old Recap datadir (before 0.7)."
    echo "Nothing to do."
    exit 0
    ;;
  2)
    echo "Detected Recap 0.7 datadir."
    ;;
  3)
    echo "Detected Recap pre-0.8 datadir."
    ;;
  4)
    echo "Detected Recap 0.8 datadir."
    ;;
esac

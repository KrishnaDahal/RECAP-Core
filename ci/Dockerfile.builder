...
# recap_hash
RUN git clone https://github.com/recappay/recap_hash
RUN cd recap_hash && python3 setup.py install
...
RUN groupadd -g ${GROUP_ID} recap
RUN useradd -u ${USER_ID} -g recap -s /bin/bash -m -d /recap recap
...
RUN mkdir /recap-src && \
  ...
  chown $USER_ID:$GROUP_ID /recap-src && \

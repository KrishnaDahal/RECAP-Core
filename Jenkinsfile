...
// which are part of the Recap repo will allow modification to the Jenkinsfile.
...
for(int i = 0; i < targets.size(); i++) {
  ...
  tasks["${target}"] = {
    node {
      ...
      withEnv(env) {
        def builderImageName="recap-builder-${target}"
        ...
        builderImage.inside("-t") {
          ...
          sh "cp -ra ${pwd}/. /recap-src/"

          // restore cache
          def hasCache = false
          try {
            copyArtifacts(projectName: "recappay-recap/${BRANCH_NAME}", optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz")
          } catch (Exception e) {
          }
          if (fileExists("ci-cache-${target}.tar.gz")) {
            hasCache = true
            echo "Using cache from recappay-recap/${BRANCH_NAME}"
          } else {
            try {
              copyArtifacts(projectName: 'recappay-recap/develop', optional: true, selector: lastSuccessful(), filter: "ci-cache-${target}.tar.gz");
            } catch (Exception e) {
            }
            if (fileExists("ci-cache-${target}.tar.gz")) {
              hasCache = true
              echo "Using cache from recappay-recap/develop"
            }
          }

          if (hasCache) {
            sh "cd /recap-src && tar xzf ${pwd}/ci-cache-${target}.tar.gz"
          } else {
            sh "mkdir -p /recap-src/ci-cache-${target}"
          }

          stage("${target}/depends") {
            sh 'cd /recap-src && ./ci/build_depends.sh'
          }
          stage("${target}/build") {
            sh 'cd /recap-src && ./ci/build_src.sh'
          }
          stage("${target}/test") {
            sh 'cd /recap-src && ./ci/test_unittests.sh'
          }
          stage("${target}/test") {
            sh 'cd /recap-src && ./ci/test_integrationtests.sh'
          }

          // archive cache and copy it into the jenkins workspace
          sh "cd /recap-src && tar czfv ci-cache-${target}.tar.gz ci-cache-${target} && cp ci-cache-${target}.tar.gz ${pwd}/"
        }

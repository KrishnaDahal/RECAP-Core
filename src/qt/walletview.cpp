...
void WalletView::processNewTransaction(const QModelIndex& parent, int start, int /*end*/)
{
    ...
    TransactionTableModel *recap = walletModel->getTransactionTableModel();
    if (!recap || recap->processingQueuedTransactions())
        return;

    QModelIndex index = recap->index(start, 0, parent);
    QSettings settings;
    if (!settings.value("fShowPrivateSendPopups").toBool()) {
        QVariant nType = recap->data(index, TransactionTableModel::TypeRole);
    ...
    QString date = recap->index(start, TransactionTableModel::Date, parent).data().toString();
    qint64 amount = recap->index(start, TransactionTableModel::Amount, parent).data(Qt::EditRole).toULongLong();
    QString type = recap->index(start, TransactionTableModel::Type, parent).data().toString();
    QString address = recap->data(index, TransactionTableModel::AddressRole).toString();
    QString label = recap->data(index, TransactionTableModel::LabelRole).toString();

...
public:
    ...
    void invoke(QObject *recap)
    {
        QString strHash = QString::fromStdString(hash.GetHex());
        qDebug() << "NotifyTransactionChanged: " + strHash + " status= " + QString::number(status);
        QMetaObject::invokeMethod(recap, "updateTransaction", Qt::QueuedConnection,
...
static void NotifyTransactionChanged(TransactionTableModel *recap, CWallet *wallet, const uint256 &hash, ChangeType status)
{
    // Find transaction in wallet
    std::map<uint256, CWalletTx>::iterator mi = wallet->mapWallet.find(hash);
    ...
    notification.invoke(recap);
}

static void NotifyAddressBookChanged(TransactionTableModel *recap, CWallet *wallet, const CTxDestination &address, const std::string &label, bool isMine, const std::string &purpose, ChangeType status)
{
    QMetaObject::invokeMethod(recap, "updateAddressBook", Qt::QueuedConnection,
...
static void ShowProgress(TransactionTableModel *recap, const std::string &title, int nProgress)
{
    if (nProgress == 0)
    ...
    if (nProgress == 100)
    {
        ...
        if (vQueueNotifications.size() > 10) // prevent balloon spam, show maximum 10 balloons
            QMetaObject::invokeMethod(recap, "setProcessingQueuedTransactions", Qt::QueuedConnection, Q_ARG(bool, true));
        for (unsigned int i = 0; i < vQueueNotifications.size(); ++i)
        {
            if (vQueueNotifications.size() - i <= 10)
                QMetaObject::invokeMethod(recap, "setProcessingQueuedTransactions", Qt::QueuedConnection, Q_ARG(bool, false));

            vQueueNotifications[i].invoke(recap);
        }

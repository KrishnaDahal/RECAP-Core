...
const char * const BITCOIN_CONF_FILENAME = "recap.conf";
const char * const BITCOIN_PID_FILENAME = "recapd.pid";
...
{
    // Windows < Vista: C:\Documents and Settings\Username\Application Data\RecapCore
    // Windows >= Vista: C:\Users\Username\AppData\Roaming\RecapCore
    // Mac: ~/Library/Application Support/RecapCore
    // Unix: ~/.recapcore
#ifdef WIN32
    // Windows
    return GetSpecialFolderPath(CSIDL_APPDATA) / "RecapCore";
#else
    fs::path pathRet;
...
#ifdef MAC_OSX
    // Mac
    return pathRet / "Library/Application Support/RecapCore";
#else
    // Unix
    return pathRet / ".recapcore";
#endif
#endif
}
...
void ArgsManager::ReadConfigFile(const std::string& confPath)
{
    ...
    if (!streamConfig.good()){
        // Create empty recap.conf if it does not excist
...

    {
        ...
        {
            // Don't overwrite existing settings so command line settings override recap.conf
